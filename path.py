...
def find_shortest_path_astar(airspace, origin, destination, max_iterations=5000, debug=False, custom_cost_func=None,
                             recursion_level=0):
    """
    ...
    if origin_is_airport or dest_is_airport:
        ...
        for o_point in origin_points:
            for d_point in dest_points:
                if debug:
                    print(
                        f"Probando ruta desde {o_point.name} (#{o_point.number}) a {d_point.name} (#{d_point.number})")
    ...
    if origin_is_airport or dest_is_airport:
        ...
        for o_point in origin_points:
            for d_point in dest_points:
                if debug:
                    print(
                        f"Probando múltiples rutas desde {o_point.name} (#{o_point.number}) a {d_point.name} (#{d_point.number})")
